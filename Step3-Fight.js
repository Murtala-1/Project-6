(function() {
  ...
  Player.prototype.refreshSummaryHtml = function() {
    ...
    <h5 class="turning-title" ${this.fighting ? '' : 'hidden'} >
      ${
        this.defending
          ? 'defending (50% less damage)'
          : 'attacking (100% damage)'
      }
    </h5>
  ...
  Gamebase.prototype.fight = function() {
    ...
    if (self.player1.health > 0 && self.player2.health > 0) {
      ...
      const anotherPlayerKey =
        self.activePlayer === 'player1' ? 'player2' : 'player1';
      ...
      setTimeout(function() {
        const response = prompt(
          activePlayer.name + `, do you want 'attack' (a) or 'defend' (d) ?`,
          'attack',
        );
        ...
        if (action === 'attack') {
          ...
          console.log(
            anotherPlayer.name + '.health before',
            anotherPlayer.health,
          );
          anotherPlayer.health -= anotherPlayer.defending
            ? activePlayer.weapon.damage * 0.5
            : activePlayer.weapon.damage;
          anotherPlayer.health = Math.max(anotherPlayer.health, 0);
          console.log(
            anotherPlayer.name + '.health after',
            anotherPlayer.health,
          );
      ...
      setTimeout(function() {
        self.player2.health <= 0 &&
          alert('CONGRATUNATION: PLAYER 1 IS WINNER !!!');
        self.player1.health <= 0 &&
          alert('CONGRATUNATION: PLAYER 2 IS WINNER !!!');
        self.setup();
      }, 0);
    }
  };
